...
use lib::{
    asset, config::postgres, device, localization, message, processing::MessagePump, source,
    subscription,
};
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    // Event sources
    source::prices::start(blockchain_updates_url, starting_height, events_tx.clone()).await?;
